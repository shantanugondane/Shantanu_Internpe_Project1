...
if selected == "Predict Price":
    ...
    kms_driven = st.number_input(
        "Kms Driven", min_value=0, max_value=10000000, step=1000
    )
...
if selected == "About":
    ...
    st.write(
        "A Car Price Prediction System leverages machine learning algorithms to predict car prices by analyzing diverse features and parameters. This system utilizes historical data encompassing car sales and prevailing market trends to extrapolate potential prices for specific vehicles. The process initiates with the comprehensive collection of a dataset containing details such as brand, model, manufacturing year, mileage, and engine specifications, alongside corresponding sale prices.Subsequent to data preprocessing to ensure consistency and handle outliers, feature selection identifies the most pertinent variables influencing car prices. The chosen machine learning algorithm is then trained on this refined dataset, discerning patterns and relationships between features and the target variable - the car prices. Evaluation metrics such as Mean Squared Error or R-squared are employed to gauge the model's accuracy on unseen data.Once trained and validated, the model serves as a predictive tool for estimating car prices based on input features. These systems offer substantial benefits to both buyers and sellers in the automotive market by providing insights into fair market values and enabling data-driven pricing strategies. Ultimately, Car Price Prediction Systems contribute to fostering a transparent and informed automotive marketplace, thereby enhancing decision-making for consumers and industry stakeholders alike.."
